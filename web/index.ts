 ...
 *
 *   • `PortalShell`/`TabbedPortalShell` here vs `AppShell` there — phone chrome is wrong on a
 *     desktop portal, and the different name is the point rather than an oversight.
 ...
 *
 * Two shells, not one: `PortalShell` for a screen with no role-scoped tabs (COA's initial screens,
 * admin-react's own pre-auth Login/ForgotPassword/Setup); `TabbedPortalShell` for a screen that
 * has them, with the per-role `TabStrip` pinned to the bottom.
 *
...
export { TabbedPortalShell } from './TabbedPortalShell';
